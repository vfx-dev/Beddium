...
#if defined(RPLE)
ivec2 _vert_tex_light_coord_r;
ivec2 _vert_tex_light_coord_g;
ivec2 _vert_tex_light_coord_b;
#else
...
#if defined(RPLE)
in uint a_LightCoord_r;
in uint a_LightCoord_g;
in uint a_LightCoord_b;
#else
...
void _vert_init() {
    ...
    #if defined(RPLE)
    uint packed_draw_params = (a_LightCoord_r & 0xFFFFu);
    #else
    ...
    // Vertex Light
    #if defined(RPLE)
    _vert_tex_light_coord_r = ivec2((uvec2((a_LightCoord_r >> 16) & 0xFFFFu) >> uvec2(0, 8)) & uvec2(0xFFu));
    _vert_tex_light_coord_g = ivec2((uvec2((a_LightCoord_g >> 16) & 0xFFFFu) >> uvec2(0, 8)) & uvec2(0xFFu));
    _vert_tex_light_coord_b = ivec2((uvec2((a_LightCoord_b >> 16) & 0xFFFFu) >> uvec2(0, 8)) & uvec2(0xFFu));
    #else
